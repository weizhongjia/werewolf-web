...
<script>
  export default{
    ...
    props: ['playerInfoList', 'selfInfo', 'sheriffVoteList'],
    methods: {
      vote: function (vote) {
        if (vote) {
      ...
      getClass: function (player) {
        let isSelf = !player.alive || Object.keys(this.sheriffVoteList).filter(number => parseInt(number) === player.seatNumber).length === 0
